...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    ...
    container.innerHTML = renderPackagePage(pkg, pkgReviews);
...
function getLinkType(url) {
  ...
  if (url.includes('gitlab.com')) return { icon: '📂', label: 'GitLab' };
  if (url.includes('r-universe.dev')) return { icon: '🌐', label: 'R-universe' };
  if (url.includes('cran.r-project.org')) return { icon: '📦', label: 'CRAN' };
  if (url.includes('bioconductor.org')) return { icon: '🧬', label: 'Bioconductor' };
  return { icon: '🔗', label: 'Website' };
}

function renderPackagePage(pkg, reviews) {
  ...
  let html = `
    <div class="pkg-header">
      ...
      <div class="pkg-title-section">
        ...
        <div class="pkg-badges">
          ${pkg.score && pkg.score > 0 ? `<span class="pkg-badge badge-score ${getScoreClass(pkg.score)}">Score: ${pkg.score.toFixed(1)}</span>` : ''}
          ${pkg.stars ? `<span class="pkg-badge badge-stars">⭐ ${pkg.stars.toLocaleString()}</span>` : ''}
    ...
    <div class="pkg-actions">
      ...
      ${bugUrl ? `<a href="${bugUrl}" target="_blank" class="pkg-action-btn btn-secondary">🐛 Report Issue</a>` : ''}
      <a href="/submit-review.html?pkg=${encodeURIComponent(pkg.package_name)}" class="pkg-action-btn btn-primary">✍️ Write Review</a>
    </div>
  `;
